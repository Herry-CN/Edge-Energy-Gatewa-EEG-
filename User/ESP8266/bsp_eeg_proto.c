/**
  ...
  * @brief   EEG V1.0 报文层实现（Doc/MQTT 网关通信协议 V1.0.md §5~§9）
  *
  * 与旧私有协议的关键差异：
  *   1. 主题从 /device/{id}/xxx 换成 eeg/{site}/{gw}/{type}/{id}/{channel}
  *   2. 数值从「整数倍率」换成带小数点的十进制（380.1 / 31.2 / 12.35）
...
void EEG_UpdateDerived(void)
{
    ...
    /* 电量积分：0.01kWh 增量 = P(0.1kW) × dt(ms) / 360000，余数留到下一轮，
     * 免得每次都往下取整把电量吃掉。 */
    if (g_current_power > 0) {
        uint32_t num = g_current_power * dt + energy_rem;
        g_energy_charge += num / 360000UL;
        energy_rem       = num % 360000UL;
    }
...
void EEG_CheckTempAlarm(void)
{
    ...
    if (!alarm && g_temperature >= EEG_TEMP_ALARM_C) {
        alarm        = true;
        g_fault_code = EEG_EVT_CODE_OVER_TEMP;
